...
// File: WinMain.cpp
//
// Desc: This is a template for createing DirectX programs
//
//-----------------------------------------------------------------------------
...
HRESULT             hr; //to hold the return value for dd functions
char                cpErrorBuf[1024]; //holds the error string which will be displayed if there is an error
...
int WINAPI WinMain(	HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdline, int nCmdShow)
{
	...
	if( !Initiate() )
		{
			MessageBox(hWnd, cpErrorBuf, "Initiation Failed!", MB_OK); //NULL is default for Error Box
			return(0);
		}
	...
	if( !Shutdown() )
		{
			MessageBox(hWnd, cpErrorBuf, "Failed to Shutdown!", NULL); //NULL is default for Error Box
			return(0);
		}		
...
int Initiate()
{
	...
	hr = lpdd->SetDisplayMode(WINDOW_WIDTH ,WINDOW_HEIGHT,8);
