...
int WINAPI WinMain(	HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdline, int nCmdShow)
{
	...
	while(!iDone)
	{
    	...
    	iDone = Main(cpErrorBuf, ddsd, lpddsprimary, lpddsback);
	...
	//If there was an error in Main say why.
	if(iDone == -1)
		MessageBox(NULL, cpErrorBuf, "Main Failed", NULL); //NULL is default for Error box
...
int Initiate()
{
	...
	// set the flags to validate both the capabilities field and the backbuffer count field
	ddsd.dwFlags = DDSD_CAPS | DDSD_BACKBUFFERCOUNT;
	// we need to let dd know that we want a complex flippable surface structure, set flags for that
	...
	// query for the backbuffer or secondary surface notice the use of ddscaps to indicate what 
	// we are requesting
	ddscaps.dwCaps = DDSCAPS_BACKBUFFER;

	// get the surface
	hr = lpddsprimary->GetAttachedSurface(&ddscaps,&lpddsback);
	if (DDFailedCheck(hr, "GetAttachedSurface()", cpErrorBuf ))
		return 0;
